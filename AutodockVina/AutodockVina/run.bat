...
cd /d "%~dp0"
REM =============================================
REM  FBKdock AutoDock Vina — Windows Starter
...
for /l %%i in (1,1,%LOOP%) do (
    ...
    if %METHOD%==1 vina.exe --config %CONFIG% --log log.txt
    if %METHOD%==2 vina.exe --config %CONFIG% --split_log
    if %METHOD%==3 vina.exe --config %CONFIG% --split_log --log_dir logs
