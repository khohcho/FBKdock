...
void SetupBuildProgramWithSource(cl_program program_cl, cl_program program_head, cl_device_id* devices, std::string include_path, std::string addtion) {
    ...
    std::string full_path = head_inc_path + include_path + option + addtion + " -cl-nv-verbose";
    const char* options = full_path.data();

    printf("\n[DEBUG] Compiling with options: %s\n", options); fflush(stdout);

    //Build program
    err = clBuildProgram(program_cl, 1, devices, options, NULL, NULL); fflush(stdout);

    // Check build status explicitly
    cl_build_status build_status;
    clGetProgramBuildInfo(program_cl, *devices, CL_PROGRAM_BUILD_STATUS, sizeof(cl_build_status), &build_status, NULL);
    printf("\n[DEBUG] Build status: %d (CL_BUILD_SUCCESS=0, CL_BUILD_ERROR=-2)\n", (int)build_status); fflush(stdout);

    // Always try to get build log, regardless of success/failure
    char* buffer = NULL;
    size_t logsize = 0;
    cl_int log_err = clGetProgramBuildInfo(program_cl, *devices, CL_PROGRAM_BUILD_LOG, 0, NULL, &logsize);
    printf("\n[DEBUG] clGetProgramBuildInfo returns %d, logsize: %zu\n", log_err, logsize); fflush(stdout);
    if (logsize > 1) {
        buffer = (char*)malloc((logsize + 1) * sizeof(char));
        clGetProgramBuildInfo(program_cl, *devices, CL_PROGRAM_BUILD_LOG, logsize, buffer, NULL);
        buffer[logsize] = '\0';
    ...
    if (CL_SUCCESS != err) {
        printf("\nError: Failed to build program executable! error code: %d", err); fflush(stdout);
        exit(-1);
    }
    else {
#ifdef  DISPLAY_SUCCESS
