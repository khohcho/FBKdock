...
int metropolis_accept(float old_f, float new_f, float temperature, float n) {
	if (new_f < old_f)return 1;
	const float acceptance_probability = exp((old_f - new_f) / temperature);
	int res = n < acceptance_probability;
...
void kernel2(
	...
	int gl = gx + gy * gs; // get_global_linear_id() is OpenCL 2.0 only
