...
#ifdef LARGE_BOX
	// docking box size <= ~52.5 Angstrom (140 * 0.375 Å)
	#define MAX_NUM_OF_GRID_MI 140
	#define MAX_NUM_OF_GRID_MJ 140
	#define MAX_NUM_OF_GRID_MK 140
	#define MAX_NUM_OF_ATOM_RELATION_COUNT 10000
	#define MAX_NUM_OF_NEIGHBORS_PER_CELL 512
#endif
...
typedef struct { // depth-first order
	int		num_children;
	int	children_map	[MAX_NUM_OF_RIGID][MAX_NUM_OF_RIGID]; // chidren_map[i][j] = 1 if node i's child is node j
...
typedef struct {
	int relation[MAX_NUM_OF_ATOM_RELATION_COUNT][MAX_NUM_OF_NEIGHBORS_PER_CELL];
